...
def plot_regression_tenth_order(params):
    ...
    plt.plot(x_line, y_line, color='blue', label=f'Regression Line: 9th Order Polynomial')
...
def calculate_parameters_tenth_order():
    ...
    c_0 = np.random.uniform(-1, 1)
    d_0 = np.random.uniform(-1, 1)
    e_0 = np.random.uniform(-1, 1)
    f_0 = np.random.uniform(-1, 1)
    g_0 = np.random.uniform(-1, 1)
    h_0 = np.random.uniform(-1, 1)
    i_0 = np.random.uniform(-1, 1)
    j_0 = np.random.uniform(-1, 1)
    ...
    while True:
        ...
        while power >= 0:
            grad_sum = 0
            for instance in dataset:
                grad_sum += (a_0*(instance[0]**9) + b_0*(instance[0]**8) + c_0*(instance[0]**7) + d_0*(instance[0]**6) +
                             e_0*(instance[0]**5) + f_0*(instance[0]**4) + g_0*(instance[0]**3) + h_0*(instance[0]**2) +
                             i_0*instance[0] + j_0 - instance[1]) * 2 * (instance[0] ** power)
        ...
        if ((a_n - a_0)**2 + (b_n - b_0)**2 + (c_n - c_0)**2 + (d_n - d_0)**2 + (e_n - e_0)**2 + (f_n - f_0)**2 +
            (g_n - g_0)**2 + (h_n - h_0)**2 + (i_n - i_0)**2 + (j_n - j_0)**2) < e:
            params = [a_n, b_n, c_n, d_n, e_n, f_n, g_n, h_n, i_n, j_n]
            break
        else:
            a_0 = a_n
            b_0 = b_n
