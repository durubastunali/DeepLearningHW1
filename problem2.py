# Duru Baştunalı 150120075 - CSE4079 HW1, Question 2
...
def plot_regression_tenth_order(a, b, c, d, e, f, g, h, i, j, k):  # Plot tenth order model
...
def plot_regression_linear(a, b):  # Plot linear model
    x_values = [point[0] for point in dataset]
    y_values = [point[1] for point in dataset]
    plt.scatter(x_values, y_values, color='red', label='Data Points')
    x_line = np.linspace(min(x_values), max(x_values), 100)
...
# Tenth order gradient with regularization
...
# Tenth order gradient without regularization
def partial_derivative_tenth(a, b, c, d, e, f, g, h, i, j, k, power):
    grad_a_sum = 0
    for instance in dataset:
        x = instance[0]
        y = instance[1]
        tenth_order = a*(x**10)+b*(x**9)+c*(x**8)+d*(x**7)+e*(x**6)+f*(x**5)+g*(x**4)+h*(x**3)+i*(x**2)+j*x+k-y
        grad_a_sum += 2 * (x ** power) * tenth_order
    return grad_a_sum


def partial_derivative_linear(a, b, power):  # Linear model gradient
    grad_a_sum = 0
    for instance in dataset:
...
def calculate_parameters_tenth_order_regularization():  # Calculate tenth order model with regularization
    a_0 = 2.41
    b_0 = 0.38
    c_0 = 0
    d_0 = 0
...
def calculate_parameters_tenth_order():  # Calculate tenth order model without regularization
    a_0 = 0
    b_0 = 0
    c_0 = 0
    d_0 = 0
...
def calculate_parameters_linear():  # Calculate linear model
    a_0 = 0
    b_0 = 0
